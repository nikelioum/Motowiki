...
const props = defineProps({
    brand: Object,  // Single brand data
    bikes: Array,   // Bikes related to the brand
    breadcrumb: Array,
});
...
// All categories for filter dropdown (from bike.category)
const categories = computed(() => {
    const allCategories = props.bikes.map(b => b.category?.title).filter(Boolean);
...
<template>
    <Head>
        ...
        <meta name="description" :content="'Δείτε τις μοτοσυκλέτες της μάρκας ' + brand.name" />
        <!-- Open Graph / Facebook -->
    ...
    <div class="py-20">
        ...
        <p class="max-w-[1200px]">{{ brand.description }}</p>

        <Link class="bg-black text-white text-base px-6 py-1 inline-flex items-center gap-2 mt-6" href="/brands">
        ...
        <div class="my-10 flex flex-wrap gap-6">
            <div>
                <label class="block mb-1 font-medium">Φίλτρο κατά Κατηγορία</label>
        ...
        <div v-if="Object.keys(groupedBikes).length">
            <div v-for="(years, category) in groupedBikes" :key="category" class="mb-10">
